...
struct detial
{
 char name[20];
 int an;
}d1={" Chitra",12345},d2={"Varsha",12346},d3={"Keerthi",12347},d4;
...
printf("TO");
gotoxy(28,18);
printf("KOWSHIK'S BANK OF INDIA");
getch();
clrscr();
star();
gotoxy(25,12);
...
{
 p[0] = fopen("chitra.txt","r");
 fscanf(p[0],"%d",&n);
 q=0;
 fclose(p[0]);
 p[0] = fopen("chitra.txt","w");
}
else if(strcmp(d2.name,d4.name)==0&&d2.an==d4.an)
{
 p[1] = fopen("varsha.txt","r");
 fscanf(p[1],"%d",&n);
 q=1;
 fclose(p[1]);
 p[1] = fopen("varsha.txt","w");
}
else if(strcmp(d3.name,d4.name)==0&&d3.an==d4.an)
{
 p[2] = fopen("keerthi.txt","r");
 fscanf(p[2],"%d",&n);
 q=2;
 fclose(p[2]);
 p[2] = fopen("keerthi.txt","w");
}
else
{
star();
...
switch(ch)
{
 ...
 case 4 : gotoxy(35,11);
	  ...
	  if(g==d1.an)
	  {
	   r = fopen("chitra.txt","r");
	   fscanf(r,"%d",&s);
	   s=s+e;
	   fclose(r);
	   r = fopen("chitra.txt","w");
	   fprintf(r,"%d",s);
	   fclose(r);
	  }
	  else if(g==d2.an)
	  {
	   r = fopen("varsha.txt","r");
	   fscanf(r,"%d",&s);
	   s=s+e;
	   fclose(r);
	   r = fopen("varsha.txt","w");
	   fprintf(r,"%d",s);
	   fclose(r);
	  }
	  else if(g==d3.an)
	  {
	   r = fopen("keerthi.txt","r");
	   fscanf(r,"%d",&s);
	   s=s+e;
	   fclose(r);
	   r = fopen("keerthi.txt","w");
	   fprintf(r,"%d",s);
	   fclose(r);
	  }
	  else
